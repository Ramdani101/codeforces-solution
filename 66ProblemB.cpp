...
public:
    ...
    void solve(){
        ...
        rep(i,n){
            ...
            while (i - left - 1 >= 0 && heights[i - left - 1] < currHeight) {
                left++;
                currHeight = heights[i - left - 1];
            }
            currHeight = heights[i];
            while (i + right + 1 < n && heights[i + right + 1] < currHeight){
                right++;
                currHeight = heights[i + right + 1];
            }
            maxDrop = max(maxDrop, left + right + 1);
        }
        outputln(maxDrop);
    }
};
